...
import { suspendCronJob } from '$lib/server/services/kubernetes';
...
export const POST: RequestHandler = async (event) => {
	...
	try {
		...
		const result = await suspendCronJob(clusterId, name, namespace, suspend);

		if (result.success) {
			await audit(event, 'update', 'cronjob', {
				entityName: name,
				clusterId,
				description: `${suspend ? 'Suspended' : 'Resumed'} cronjob ${name} in namespace ${namespace}`
			});
		}

		return json(result);
	} catch (error) {
		console.error('[API] Error suspending cronjob:', error);
		return json(
			{
				success: false,
				error: error instanceof Error ? error.message : 'Failed to suspend cronjob'
			},
			{ status: 500 }
		);
	}
