...
import { deleteCronJob } from '$lib/server/services/kubernetes';
...
export const DELETE: RequestHandler = async (event) => {
	...
	try {
		...
		const cronJobName = url.searchParams.get('name');
		...
		const result = await deleteCronJob(clusterId, cronJobName, namespace);

		if (result.success) {
			await audit(event, 'delete', 'cronjob', {
				entityName: cronJobName,
				clusterId,
				description: `Deleted cronjob ${cronJobName} in namespace ${namespace}`
			});
		}

		return json(result);
	} catch (error) {
		console.error('[API] Error deleting cronjob:', error);
		return json(
			{
				success: false,
				error: error instanceof Error ? error.message : 'Failed to delete cronjob'
			},
			{ status: 500 }
		);
	}
