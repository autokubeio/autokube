...
import { listCronJobs } from '$lib/server/services/kubernetes';
...
export const GET: RequestHandler = async ({ params, url, cookies }) => {
	...
	if (auth.authEnabled && !await auth.can('jobs', 'read', clusterId)) {
	...
	const result = await listCronJobs(clusterId, ns);

	if (!result.success) {
		return json({ success: false, error: result.error }, { status: 500 });
	}

	return json({ success: true, cronJobs: result.cronJobs });
};
