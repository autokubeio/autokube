...
import { listPersistentVolumeClaims } from '$lib/server/services/kubernetes';
import { authorize } from '$lib/server/services/authorize';

/**
 * GET /api/clusters/[id]/persistentvolumeclaims
 * Query params:
...
export const GET: RequestHandler = async ({ params, url, cookies }) => {
	...
	const result = await listPersistentVolumeClaims(clusterId, ns);

	if (!result.success) {
		return json({ success: false, error: result.error }, { status: 500 });
	}

	return json({ success: true, persistentVolumeClaims: result.persistentVolumeClaims });
};
