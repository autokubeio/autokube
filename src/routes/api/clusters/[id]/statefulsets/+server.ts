...
import { listStatefulSets } from '$lib/server/services/kubernetes';
...
export const GET: RequestHandler = async ({ params, url, cookies }) => {
	...
	const result = await listStatefulSets(clusterId, ns);

	if (!result.success) {
		return json({ success: false, error: result.error }, { status: 500 });
	}

	return json({ success: true, statefulSets: result.statefulSets });
};
