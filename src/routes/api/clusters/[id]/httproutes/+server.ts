...
import { listHTTPRoutes } from '$lib/server/services/kubernetes';
...
export const GET: RequestHandler = async ({ params, url, cookies }) => {
	...
	try {
		const result = await listHTTPRoutes(clusterId, ns);
		if (!result.success) {
			return json(
				{ success: false, error: result.error || 'Failed to fetch HTTPRoutes' },
				{ status: 500 }
			);
		}
		return json({
		...
		console.error('[API] Failed to list HTTPRoutes:', err);
