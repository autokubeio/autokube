...
import { deleteDeployment } from '$lib/server/services/kubernetes';
...
export const DELETE: RequestHandler = async (event) => {
	...
	try {
		...
		const deploymentName = url.searchParams.get('name');
		...
		const result = await deleteDeployment(clusterId, deploymentName, namespace);

		if (result.success) {
			await audit(event, 'delete', 'deployment', {
				entityName: deploymentName,
				clusterId,
				description: `Deleted deployment ${deploymentName} in namespace ${namespace}`
			});
		}

		return json(result);
	} catch (error) {
		console.error('[API] Error deleting deployment:', error);
		return json(
			{
				success: false,
