...
import { deletePod } from '$lib/server/services/kubernetes';
...
export const DELETE: RequestHandler = async (event) => {
	...
	try {
		...
		const podName = url.searchParams.get('name');
		...
		if (!podName) {
			return json({ success: false, error: 'Pod name is required' }, { status: 400 });
		}
		...
		const result = await deletePod(clusterId, podName, namespace);

		if (result.success) {
			await audit(event, 'delete', 'pod', {
				entityName: podName,
				clusterId,
				description: `Deleted pod ${podName} in namespace ${namespace}`
			});
		}
