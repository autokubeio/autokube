...
import { deleteJob } from '$lib/server/services/kubernetes';
...
export const DELETE: RequestHandler = async (event) => {
	...
	try {
		...
		const result = await deleteJob(clusterId, jobName, namespace);

		if (result.success) {
			await audit(event, 'delete', 'job', {
				entityName: jobName,
				clusterId,
				description: `Deleted job ${jobName} in namespace ${namespace}`
			});
		}
