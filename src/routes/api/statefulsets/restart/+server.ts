...
import { restartStatefulSet } from '$lib/server/services/kubernetes';
...
export const POST: RequestHandler = async (event) => {
	...
	try {
		...
		const result = await restartStatefulSet(clusterId, name, namespace);

		if (result.success) {
			await audit(event, 'update', 'statefulset', {
				entityName: name,
				clusterId,
				description: `Restarted statefulset ${name} in namespace ${namespace}`
			});
		}

		return json(result);
	} catch (error) {
		console.error('[API] Error restarting statefulset:', error);
		return json(
			{
				success: false,
				error: error instanceof Error ? error.message : 'Failed to restart statefulset'
			},
			{ status: 500 }
		);
	}
