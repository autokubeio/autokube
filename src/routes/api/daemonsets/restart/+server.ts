...
import { restartDaemonSet } from '$lib/server/services/kubernetes';
...
export const POST: RequestHandler = async (event) => {
	...
	try {
		...
		const result = await restartDaemonSet(clusterId, name, namespace);

		if (result.success) {
			await audit(event, 'update', 'daemonset', {
				entityName: name,
				clusterId,
				description: `Restarted daemonset ${name} in namespace ${namespace}`
			});
		}

		return json(result);
	} catch (error) {
		console.error('[API] Error restarting daemonset:', error);
		return json(
			{
				success: false,
				error: error instanceof Error ? error.message : 'Failed to restart daemonset'
			},
			{ status: 500 }
		);
	}
