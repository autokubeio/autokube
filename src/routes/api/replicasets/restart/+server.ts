...
import { restartReplicaSet } from '$lib/server/services/kubernetes';
...
export const POST: RequestHandler = async (event) => {
	...
	try {
		...
		const result = await restartReplicaSet(clusterId, name, namespace);

		if (result.success) {
			await audit(event, 'update', 'replicaset', {
				entityName: name,
				clusterId,
				description: `Restarted replicaset ${name} in namespace ${namespace}`
			});
		}

		return json(result);
	} catch (error) {
		console.error('[API] Error restarting replicaset:', error);
		return json(
			{
				success: false,
				error: error instanceof Error ? error.message : 'Failed to restart replicaset'
			},
			{ status: 500 }
		);
	}
