<script lang="ts">
	...
	import type { ReferenceGrantInfo } from '$lib/server/services/kubernetes';
	import { DataTableView, type DataTableSortState } from '$lib/components/data-table-view';
	import { TableName, referenceGrantsColumns } from '$lib/table-columns';
	import { toast } from 'svelte-sonner';
	import ResourceDrawer, { type ResourceRef } from '$lib/components/resource-drawer.svelte';

	type Row = ReferenceGrantInfo & { age: string };

	const activeCluster = $derived(clusterStore.active);
	const activeClusterId = $derived(clusterStore.active?.id ?? null);
	let allItems = $state<ReferenceGrantInfo[]>([]);
		...
		if (searchQuery.trim()) {
			...
			result = result.filter(
				(r) =>
					...
					r.to.some((t) => t.toLowerCase().includes(q))
			);
		}
		if (sortState) {
			result = arraySort(result, sortState.field as keyof ReferenceGrantInfo, sortState.direction, {
	...
	let watchHandle: ReturnType<typeof useBatchWatch<ReferenceGrantInfo>> | null = null;
	...
	$effect(() => {
		...
		if (clusterId) {
			...
			watchHandle = useBatchWatch<ReferenceGrantInfo>({
				clusterId,
	...
	async function fetchNamespaces(clusterId: number) {
		try {
			...
			console.error('[ReferenceGrants] Failed to fetch namespaces:', err);
		}
	}
	...
	async function fetchItems(clusterId: number, nsParam: string) {
		...
		try {
			...
			const res = await fetch(`/api/clusters/${clusterId}/referencegrants?namespace=${ns}`);
			const data = await res.json();
			if (data.success) {
				allItems = data.referenceGrants ?? [];
				crdMissing = !!data.crdMissing;
			} else {
				error = data.error || 'Failed to fetch ReferenceGrants';
				allItems = [];
			}
		} catch (err) {
			error = err instanceof Error ? err.message : 'Failed to fetch ReferenceGrants';
			allItems = [];
		} finally {
			loading = false;
		}
	...
	async function handleDelete(name: string, namespace: string) {
		...
		try {
			...
			const response = await fetch(
				`/api/clusters/${activeCluster.id}/referencegrants/${encodeURIComponent(name)}?namespace=${encodeURIComponent(namespace)}`,
				{ method: 'DELETE' }
			);
			if (!response.ok) {
				const data = await response.json();
				throw new Error(data.error || 'Failed to delete ReferenceGrant');
			}
			toast.success(`ReferenceGrant "${name}" deleted`);
		} catch (err: unknown) {
	...
	function openYaml(r: Row) {
		drawerResource = { resourceType: 'referencegrant', name: r.name, namespace: r.namespace };
		showYamlDialog = true;
	}
...
<svelte:head><title>Reference Grants - AutoKube</title></svelte:head>
...
<section class="flex h-full min-h-0 flex-1 flex-col">
	<div class="mb-4 flex shrink-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
		<div class="flex items-center gap-2">
			<h1 class="text-2xl font-bold">Reference Grants</h1>
	...
	{:else if !loading && !error && crdMissing}
		<div class="flex flex-1 flex-col items-center justify-center gap-3 text-center">
			<div class="flex size-12 items-center justify-center rounded-full bg-muted">
				<FileKey class="size-6 text-muted-foreground" />
			</div>
			<div>
	...
	{:else if !loading && !error && allItems.length === 0}
		<div class="flex flex-1 flex-col items-center justify-center gap-3 text-center">
			...
			<div>
				<h3 class="mb-1 font-semibold">No ReferenceGrants found</h3>
				<p class="text-sm text-muted-foreground">
					{selectedNamespace === 'all'
						? 'This cluster has no ReferenceGrants'
						: `No ReferenceGrants in namespace "${selectedNamespace}"`}
				</p>
			</div>
		</div>
	{:else}
		<div class="flex min-h-0 flex-1">
			<DataTableView
				data={filteredRows}
				keyField="id"
				name={TableName.referencegrants}
				columns={referenceGrantsColumns}
				{sortState}
			...
			>
				{#snippet cell(column, r: Row)}
					{#if column.id === 'name'}
						<div class="flex items-center gap-1.5 text-sm font-medium">
							<FileKey class="h-3.5 w-3.5 shrink-0 text-muted-foreground" />
					...
					{:else if column.id === 'from'}
						<span class="truncate font-mono text-xs" title={r.from.join(', ')}>
							{r.from.length === 0
								? '—'
								: r.from.slice(0, 2).join(', ') +
									(r.from.length > 2 ? ` +${r.from.length - 2}` : '')}
						</span>
					{:else if column.id === 'to'}
						<span class="truncate font-mono text-xs" title={r.to.join(', ')}>
							{r.to.length === 0
								? '—'
								: r.to.slice(0, 2).join(', ') + (r.to.length > 2 ? ` +${r.to.length - 2}` : '')}
						</span>
				...
				{#snippet emptyState()}
					<div class="flex flex-col items-center justify-center py-16 text-muted-foreground">
						<FileKey class="mb-3 h-10 w-10 opacity-40" />
						<p>No ReferenceGrants found</p>
					</div>
				{/snippet}
