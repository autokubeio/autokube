<script lang="ts">
	...
	import { RefreshCw, Search, AlertCircle, Globe, Trash2, Loader2, FileCode } from 'lucide-svelte';
	import { clusterStore } from '$lib/stores/cluster.svelte';
	import { useBatchWatch } from '$lib/hooks/use-batch-watch.svelte';
	import { onDestroy } from 'svelte';
	import type { HTTPRouteInfo } from '$lib/server/services/kubernetes';
	import { DataTableView, type DataTableSortState } from '$lib/components/data-table-view';
	import { TableName, httpRoutesColumns } from '$lib/table-columns';
	import { toast } from 'svelte-sonner';
	import ResourceDrawer, { type ResourceRef } from '$lib/components/resource-drawer.svelte';

	type Row = HTTPRouteInfo & { age: string };

	const activeCluster = $derived(clusterStore.active);
	const activeClusterId = $derived(clusterStore.active?.id ?? null);
	let allItems = $state<HTTPRouteInfo[]>([]);
	...
	const filteredRows = $derived.by(() => {
		...
		if (sortState) {
			result = arraySort(result, sortState.field as keyof HTTPRouteInfo, sortState.direction, {
	...
	let watchHandle: ReturnType<typeof useBatchWatch<HTTPRouteInfo>> | null = null;
	...
	$effect(() => {
		...
		if (clusterId) {
			...
			watchHandle = useBatchWatch<HTTPRouteInfo>({
				clusterId,
	...
	async function fetchNamespaces(clusterId: number) {
		try {
			...
			console.error('[HTTPRoutes] Failed to fetch namespaces:', err);
		}
	}
	...
	async function fetchItems(clusterId: number, nsParam: string) {
		...
		try {
			...
			const res = await fetch(`/api/clusters/${clusterId}/httproutes?namespace=${ns}`);
			const data = await res.json();
			if (data.success) {
				allItems = data.httpRoutes ?? [];
				crdMissing = !!data.crdMissing;
			} else {
				error = data.error || 'Failed to fetch HTTPRoutes';
				allItems = [];
			}
		} catch (err) {
			error = err instanceof Error ? err.message : 'Failed to fetch HTTPRoutes';
			allItems = [];
		} finally {
			loading = false;
		}
	...
	async function handleDelete(name: string, namespace: string) {
		...
		try {
			...
			const response = await fetch(
				`/api/clusters/${activeCluster.id}/httproutes/${encodeURIComponent(name)}?namespace=${encodeURIComponent(namespace)}`,
				{ method: 'DELETE' }
			);
			if (!response.ok) {
				const data = await response.json();
				throw new Error(data.error || 'Failed to delete HTTPRoute');
			}
			toast.success(`HTTPRoute "${name}" deleted`);
		} catch (err: unknown) {
	...
	function openYaml(r: Row) {
		drawerResource = { resourceType: 'httproute', name: r.name, namespace: r.namespace };
		showYamlDialog = true;
	}
...
<svelte:head><title>HTTP Routes - AutoKube</title></svelte:head>
...
<section class="flex h-full min-h-0 flex-1 flex-col">
	<div class="mb-4 flex shrink-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
		<div class="flex items-center gap-2">
			<h1 class="text-2xl font-bold">HTTP Routes</h1>
	...
	{:else if !loading && !error && allItems.length === 0}
		<div class="flex flex-1 flex-col items-center justify-center gap-3 text-center">
			...
			<div>
				<h3 class="mb-1 font-semibold">No HTTPRoutes found</h3>
				<p class="text-sm text-muted-foreground">
					{selectedNamespace === 'all'
						? 'This cluster has no HTTPRoutes'
						: `No HTTPRoutes in namespace "${selectedNamespace}"`}
				</p>
			</div>
		</div>
	{:else}
		<div class="flex min-h-0 flex-1">
			<DataTableView
				data={filteredRows}
				keyField="id"
				name={TableName.httproutes}
				columns={httpRoutesColumns}
				{sortState}
