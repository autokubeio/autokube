<script lang="ts">
	import { Button } from '$lib/components/ui/button';
	import { Input } from '$lib/components/ui/input';
	import * as Dialog from '$lib/components/ui/dialog';
	import { Badge } from '$lib/components/ui/badge';
	import ConfirmDelete from '$lib/components/confirm-delete.svelte';
	import { cn } from '$lib/utils';
	import { formatCreatedAt, tryPrettyJson } from '$lib/utils/formatters';
	import { arraySort } from '$lib/utils/arrays';
	import { createTimeTicker, calculateAgeWithTicker } from '$lib/utils/time-ticker.svelte';
	import {
		RefreshCw,
		Search,
		AlertCircle,
		ShieldCheck,
		Info,
		Trash2,
		Loader2,
		FileCode
	} from 'lucide-svelte';
	import { clusterStore } from '$lib/stores/cluster.svelte';
	import { useBatchWatch } from '$lib/hooks/use-batch-watch.svelte';
	import { onDestroy } from 'svelte';
	import {
		type ClusterRole,
		type ClusterRoleWithAge,
		formatVerbs,
		formatResources,
		formatApiGroups
	} from './columns';
	import { DataTableView, type DataTableSortState } from '$lib/components/data-table-view';
	import { TableName, clusterRolesColumns } from '$lib/table-columns';
	import { toast } from 'svelte-sonner';
	import ResourceDrawer, { type ResourceRef } from '$lib/components/resource-drawer.svelte';

	const activeCluster = $derived(clusterStore.active);
	const activeClusterId = $derived(clusterStore.active?.id ?? null);
	let allClusterRoles = $state<ClusterRole[]>([]);
	let loading = $state(false);
	let error = $state<string | null>(null);
	let searchQuery = $state('');

	// Search debounce
	let _searchTimer: ReturnType<typeof setTimeout> | null = null;
	function scheduleSearch(value: string) {
		if (_searchTimer !== null) clearTimeout(_searchTimer);
		_searchTimer = setTimeout(() => { searchQuery = value; }, 150);
	}

	// Detail dialog
	let showDetailDialog = $state(false);
	let selectedCR = $state<ClusterRoleWithAge | null>(null);
	let deleting = $state(false);

	// YAML editor
	let showYamlDialog = $state(false);
	let drawerResource = $state<ResourceRef | null>(null);

	// Time ticker
	const timeTicker = createTimeTicker(10000);

	// Sort state
	let sortState = $state<DataTableSortState | undefined>(undefined);

	// ClusterRoles with age
	const crsWithAge = $derived.by((): ClusterRoleWithAge[] => {
		const currentTime = timeTicker.now;
		return allClusterRoles.map((cr) => ({
			...cr,
			age: calculateAgeWithTicker(cr.createdAt, currentTime)
		}));
	});

	// Filtered
	const filteredCRs = $derived.by(() => {
		let result = crsWithAge;

		if (searchQuery.trim()) {
			const query = searchQuery.toLowerCase();
			result = result.filter((cr) => cr.name.toLowerCase().includes(query));
		}

		if (sortState) {
			result = arraySort(result, sortState.field as keyof ClusterRole, sortState.direction, {
				createdAt: (val: string) => new Date(val).getTime()
			});
		}

		return result;
	});

	// SSE watch
	let crsWatch: ReturnType<typeof useBatchWatch<ClusterRole>> | null = null;

	$effect(() => {
		const clusterId = activeClusterId;
		if (clusterId) {
			fetchClusterRoles(clusterId);

			if (crsWatch) crsWatch.unsubscribe();

			crsWatch = useBatchWatch<ClusterRole>({


				clusterId,


				resourceType: 'clusterroles',


				getItems: () => allClusterRoles,


				setItems: (v) => { allClusterRoles = v; },


				keyFn: (i) => i.name


			});

			crsWatch.subscribe();
		} else {
			allClusterRoles = [];
			if (crsWatch) {
				crsWatch.unsubscribe();
				crsWatch = null;
			}
		}
	});

	onDestroy(() => {
		crsWatch?.unsubscribe();
		timeTicker.stop();
	});

	async function fetchClusterRoles(clusterId: number) {
		loading = true;
		error = null;

		try {
			const res = await fetch(`/api/clusters/${clusterId}/clusterroles`);
			const data = await res.json();

			if (data.success && data.clusterRoles) {
				allClusterRoles = data.clusterRoles;
			} else {
				error = data.error || 'Failed to fetch cluster roles';
				allClusterRoles = [];
			}
		} catch (err) {
			error = err instanceof Error ? err.message : 'Failed to fetch cluster roles';
			allClusterRoles = [];
		} finally {
			loading = false;
		}
	}

	async function handleDelete(name: string) {
		if (!activeCluster?.id) return;

		try {
			deleting = true;
			const response = await fetch(
				`/api/clusters/${activeCluster.id}/clusterroles/${encodeURIComponent(name)}`,
				{ method: 'DELETE' }
			);
			if (!response.ok) {
				const data = await response.json();
				throw new Error(data.error || 'Failed to delete cluster role');
			}
			toast.success(`Cluster role "${name}" deleted`);
		} catch (err: any) {
			toast.error(`Failed to delete cluster role: ${err.message}`);
		} finally {
			deleting = false;
		}
	}

	function openDetail(cr: ClusterRoleWithAge) {
		selectedCR = cr;
		showDetailDialog = true;
	}

	function openYamlEditor(cr: ClusterRoleWithAge) {
		drawerResource = { resourceType: 'clusterrole', name: cr.name };
		showYamlDialog = true;
	}

	function closeYamlEditor() {
		showYamlDialog = false;
		drawerResource = null;
	}

	function handleYamlSuccess() {
		if (activeClusterId) fetchClusterRoles(activeClusterId);
	}
</script>

<svelte:head>
	<title>Cluster Roles - AutoKube</title>
</svelte:head>

<section class="flex h-full min-h-0 flex-1 flex-col">
	<!-- Toolbar -->
	<div class="mb-4 flex shrink-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
		<div class="flex items-center gap-2">
			<h1 class="text-2xl font-bold">Cluster Roles</h1>
			<span class="text-sm text-muted-foreground">
				{filteredCRs.length} of {crsWithAge.length}
			</span>
			<Button
				variant="outline"
				size="sm"
				class="h-7 gap-1.5 text-xs"
				disabled={loading || !activeCluster}
				onclick={() => { if (activeClusterId) fetchClusterRoles(activeClusterId); }}
			>
				<RefreshCw class={cn('size-3', loading && 'animate-spin')} />
				Refresh
			</Button>
		</div>
		<div class="flex items-center gap-2">
			<div class="relative flex-1 sm:flex-none">
				<Search
					class="pointer-events-none absolute top-1/2 left-2.5 size-3.5 -translate-y-1/2 text-muted-foreground"
				/>
				<Input
					placeholder="Search cluster roles..."
					class="h-8 w-full pl-8 text-xs sm:w-56"
					value={searchQuery}
					oninput={(e) => scheduleSearch(e.currentTarget.value)}
				/>
			</div>
		</div>
	</div>

	<!-- Error -->
	{#if error}
		<div
			class="mb-4 flex items-center gap-2 rounded-lg border border-destructive/50 bg-destructive/10 p-4 text-destructive"
		>
			<AlertCircle class="size-4 shrink-0" />
			<span class="text-sm">{error}</span>
		</div>
	{/if}

	<!-- Empty states -->
	{#if !loading && !error && !activeCluster}
		<div class="flex flex-1 flex-col items-center justify-center gap-3 text-center">
			<div class="flex size-12 items-center justify-center rounded-full bg-muted">
				<Search class="size-6 text-muted-foreground" />
			</div>
			<div>
				<h3 class="mb-1 font-semibold">No cluster selected</h3>
				<p class="text-sm text-muted-foreground">Select a cluster to view cluster roles</p>
			</div>
		</div>
	{:else}
		<div class="flex min-h-0 flex-1">
			<DataTableView
				data={filteredCRs}
				keyField="id"
				name={TableName.clusterroles}
				columns={clusterRolesColumns}
				{sortState}
				onSortChange={(state) => (sortState = state)}
				onRowClick={openDetail}
				wrapperClass="border rounded-lg"
				virtualScroll={true}
			>
				{#snippet cell(column, cr: ClusterRoleWithAge, rowState)}
					{#if column.id === 'name'}
						<div class="flex items-center gap-1.5 text-sm font-medium">
							<ShieldCheck class="h-3.5 w-3.5 shrink-0 text-muted-foreground" />
							<span class="truncate">{cr.name}</span>
							{#if cr.aggregationRule}
								<Badge
									variant="secondary"
									class="ml-1 px-1 py-0 text-[10px]"
								>
									aggregated
								</Badge>
							{/if}
						</div>
					{:else if column.id === 'rules'}
						<span class="text-xs font-medium">{cr.rules.length}</span>
					{:else if column.id === 'age'}
						<span class="text-xs text-muted-foreground">{cr.age}</span>
					{:else if column.id === 'actions'}
						<div class="flex items-center justify-end gap-0.5">
							<Button
								variant="ghost"
								size="icon"
								class="h-6 w-6 text-muted-foreground hover:cursor-pointer"
								onclick={(e) => {
									e.stopPropagation();
									openYamlEditor(cr);
								}}
								title="Edit YAML"
							>
								<FileCode class="h-3.5 w-3.5" />
							</Button>
							<Button
								variant="ghost"
								size="icon"
								class="h-6 w-6 text-muted-foreground hover:cursor-pointer"
								onclick={(e) => {
									e.stopPropagation();
									openDetail(cr);
								}}
								title="View details"
							>
								<Info class="h-3.5 w-3.5" />
							</Button>
							<ConfirmDelete
								title={cr.name}
								loading={deleting}
								onConfirm={() => handleDelete(cr.name)}
							>
								<Button
									variant="ghost"
									size="icon"
									disabled={deleting}
									class="h-6 w-6 text-muted-foreground hover:text-destructive hover:cursor-pointer"
									title="Delete cluster role"
								>
									<Trash2 class="h-3.5 w-3.5" />
								</Button>
							</ConfirmDelete>
						</div>
					{/if}
				{/snippet}

				{#snippet emptyState()}
					<div class="flex flex-col items-center justify-center py-16 text-muted-foreground">
						<div class="mb-3 flex size-12 items-center justify-center rounded-full bg-muted">
							<ShieldCheck class="size-6 text-muted-foreground" />
						</div>
						<h3 class="mb-1 font-semibold text-foreground">No cluster roles found</h3>
						<p class="text-sm">This cluster has no cluster roles</p>
					</div>
				{/snippet}

				{#snippet loadingState()}
					<div class="flex items-center justify-center py-16 text-muted-foreground">
						<Loader2 class="mr-2 h-5 w-5 animate-spin" />
						Loading cluster roles...
					</div>
				{/snippet}
			</DataTableView>
		</div>
	{/if}
</section>

<!-- Detail Dialog -->
<Dialog.Root bind:open={showDetailDialog}>
	<Dialog.Content class="max-h-[90vh] max-w-4xl overflow-y-auto">
		<Dialog.Header>
			<Dialog.Title>Cluster Role Details</Dialog.Title>
		</Dialog.Header>
		{#if selectedCR}
			<div class="space-y-6">
				<!-- Overview -->
				<div>
					<h3 class="mb-3 text-sm font-semibold">Overview</h3>
					<div class="grid grid-cols-2 gap-4">
						<div>
							<p class="text-sm font-medium text-muted-foreground">Name</p>
							<p class="mt-1 font-mono text-sm">{selectedCR.name}</p>
						</div>
						<div>
							<p class="text-sm font-medium text-muted-foreground">Rules</p>
							<p class="mt-1 text-sm font-medium">{selectedCR.rules.length}</p>
						</div>
						<div>
							<p class="text-sm font-medium text-muted-foreground">Aggregated</p>
							<p class="mt-1 text-sm">{selectedCR.aggregationRule ? 'Yes' : 'No'}</p>
						</div>
						<div>
							<p class="text-sm font-medium text-muted-foreground">Age</p>
							<p class="mt-1 text-sm">{selectedCR.age}</p>
						</div>
						<div class="col-span-2">
							<p class="text-sm font-medium text-muted-foreground">Created</p>
							<p class="mt-1 font-mono text-sm">
								{formatCreatedAt(selectedCR.createdAt)}
							</p>
						</div>
					</div>
				</div>

				<!-- Aggregation Rule -->
				{#if selectedCR.aggregationRule?.clusterRoleSelectors?.length}
					<div>
						<h3 class="mb-3 text-sm font-semibold">Aggregation Rule Selectors</h3>
						<div class="space-y-2">
							{#each selectedCR.aggregationRule.clusterRoleSelectors as selector, i}
								<div class="rounded-md border bg-muted/40 px-3 py-2">
									<p class="text-xs font-medium text-muted-foreground">Selector {i + 1}</p>
									{#if selector.matchLabels}
										<div class="mt-1 flex flex-wrap gap-1">
											{#each Object.entries(selector.matchLabels) as [k, v]}
												<Badge variant="secondary" class="px-1.5 py-0 font-mono text-[10px]">
													{k}={v}
												</Badge>
											{/each}
										</div>
									{/if}
								</div>
							{/each}
						</div>
					</div>
				{/if}

				<!-- Rules -->
				<div>
					<h3 class="mb-3 text-sm font-semibold">Rules</h3>
					{#if selectedCR.rules.length > 0}
						<div class="max-h-72 space-y-2 overflow-y-auto">
							{#each selectedCR.rules as rule, i}
								<div class="rounded-md border bg-muted/40 px-3 py-2">
									<div class="grid grid-cols-3 gap-2 text-xs">
										<div>
											<span class="font-medium text-muted-foreground">API Groups</span>
											<p class="mt-0.5 font-mono">{formatApiGroups(rule.apiGroups)}</p>
										</div>
										<div>
											<span class="font-medium text-muted-foreground">Resources</span>
											<p class="mt-0.5 font-mono">{formatResources(rule.resources)}</p>
										</div>
										<div>
											<span class="font-medium text-muted-foreground">Verbs</span>
											<p class="mt-0.5 font-mono">{formatVerbs(rule.verbs)}</p>
										</div>
									</div>
								</div>
							{/each}
						</div>
					{:else}
						<p class="text-sm text-muted-foreground">No rules defined</p>
					{/if}
				</div>

				<!-- Labels -->
				<div>
					<h3 class="mb-3 text-sm font-semibold">Labels</h3>
					{#if Object.keys(selectedCR.labels ?? {}).length > 0}
						<div class="max-h-48 space-y-1 overflow-y-auto">
							{#each Object.entries(selectedCR.labels) as [k, v]}
								<div class="flex items-start gap-2 text-xs">
									<span class="min-w-0 font-mono break-all text-muted-foreground">{k}:</span>
									<span class="font-mono">{v}</span>
								</div>
							{/each}
						</div>
					{:else}
						<p class="text-sm text-muted-foreground">No labels</p>
					{/if}
				</div>

				<!-- Annotations -->
				<div>
					<h3 class="mb-3 text-sm font-semibold">Annotations</h3>
					{#if Object.keys(selectedCR.annotations ?? {}).length > 0}
						<div class="max-h-72 space-y-2 overflow-y-auto pr-1">
							{#each Object.entries(selectedCR.annotations) as [k, v]}
								{@const parsed = tryPrettyJson(v)}
								<div class="rounded-md border bg-muted/40 px-3 py-2">
									<p class="mb-1 font-mono text-[11px] break-all text-muted-foreground">{k}</p>
									{#if parsed.pretty}
										<pre
											class="overflow-x-auto font-mono text-[11px] leading-relaxed break-all whitespace-pre-wrap">{parsed.text}</pre>
									{:else}
										<p class="font-mono text-xs break-all">{v}</p>
									{/if}
								</div>
							{/each}
						</div>
					{:else}
						<p class="text-sm text-muted-foreground">No annotations</p>
					{/if}
				</div>
			</div>
		{/if}
	</Dialog.Content>
</Dialog.Root>

{#if activeCluster && showYamlDialog}
	<ResourceDrawer
		bind:open={showYamlDialog}
		clusterId={activeCluster.id}
		resource={drawerResource}
		onClose={closeYamlEditor}
		onSuccess={handleYamlSuccess}
	/>
{/if}
