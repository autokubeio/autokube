<script lang="ts">
	import { Button } from '$lib/components/ui/button';
	import { Input } from '$lib/components/ui/input';
	import * as Dialog from '$lib/components/ui/dialog';
	import { Badge } from '$lib/components/ui/badge';
	import ConfirmDelete from '$lib/components/confirm-delete.svelte';
	import { cn } from '$lib/utils';
	import { formatCreatedAt, tryPrettyJson } from '$lib/utils/formatters';
	import { arraySort } from '$lib/utils/arrays';
	import { createTimeTicker, calculateAgeWithTicker } from '$lib/utils/time-ticker.svelte';
	import {
		RefreshCw,
		Search,
		AlertCircle,
		Layers,
		Info,
		Trash2,
		Loader2,
		FileCode,
		Star,
		Check,
		X
	} from 'lucide-svelte';
	import { clusterStore } from '$lib/stores/cluster.svelte';
	import { useBatchWatch } from '$lib/hooks/use-batch-watch.svelte';
	import { onDestroy } from 'svelte';
	import {
		type StorageClass,
		type StorageClassWithAge,
		getReclaimPolicyColor,
		getBindingModeColor,
		formatBindingMode
	} from './columns';
	import { DataTableView, type DataTableSortState } from '$lib/components/data-table-view';
	import { TableName, storageClassesColumns } from '$lib/table-columns';
	import { toast } from 'svelte-sonner';
	import ResourceDrawer, { type ResourceRef } from '$lib/components/resource-drawer.svelte';

	const activeCluster = $derived(clusterStore.active);
	const activeClusterId = $derived(clusterStore.active?.id ?? null);
	let allStorageClasses = $state<StorageClass[]>([]);
	let loading = $state(false);
	let error = $state<string | null>(null);
	let searchQuery = $state('');

	// Search debounce
	let _searchTimer: ReturnType<typeof setTimeout> | null = null;
	function scheduleSearch(value: string) {
		if (_searchTimer !== null) clearTimeout(_searchTimer);
		_searchTimer = setTimeout(() => { searchQuery = value; }, 150);
	}

	// Detail dialog
	let showDetailDialog = $state(false);
	let selectedSC = $state<StorageClassWithAge | null>(null);
	let deleting = $state(false);

	// YAML editor
	let showYamlDialog = $state(false);
	let drawerResource = $state<ResourceRef | null>(null);

	// Time ticker
	const timeTicker = createTimeTicker(10000);

	// Sort state
	let sortState = $state<DataTableSortState | undefined>(undefined);

	// SCs with age
	const scsWithAge = $derived.by((): StorageClassWithAge[] => {
		const currentTime = timeTicker.now;
		return allStorageClasses.map((sc) => ({
			...sc,
			age: calculateAgeWithTicker(sc.createdAt, currentTime)
		}));
	});

	// Filtered
	const filteredSCs = $derived.by(() => {
		let result = scsWithAge;

		if (searchQuery.trim()) {
			const query = searchQuery.toLowerCase();
			result = result.filter(
				(sc) =>
					sc.name.toLowerCase().includes(query) ||
					sc.provisioner.toLowerCase().includes(query) ||
					sc.reclaimPolicy.toLowerCase().includes(query) ||
					sc.volumeBindingMode.toLowerCase().includes(query)
			);
		}

		if (sortState) {
			result = arraySort(result, sortState.field as keyof StorageClass, sortState.direction, {
				createdAt: (val: string) => new Date(val).getTime()
			});
		}

		return result;
	});

	// SSE watch
	let scsWatch: ReturnType<typeof useBatchWatch<StorageClass>> | null = null;

	$effect(() => {
		const clusterId = activeClusterId;
		if (clusterId) {
			fetchStorageClasses(clusterId);

			if (scsWatch) scsWatch.unsubscribe();

			scsWatch = useBatchWatch<StorageClass>({


				clusterId,


				resourceType: 'storageclasses',


				getItems: () => allStorageClasses,


				setItems: (v) => { allStorageClasses = v; },


				keyFn: (i) => i.name


			});

			scsWatch.subscribe();
		} else {
			allStorageClasses = [];
			if (scsWatch) {
				scsWatch.unsubscribe();
				scsWatch = null;
			}
		}
	});

	onDestroy(() => {
		scsWatch?.unsubscribe();
		timeTicker.stop();
	});

	async function fetchStorageClasses(clusterId: number) {
		loading = true;
		error = null;

		try {
			const res = await fetch(`/api/clusters/${clusterId}/storageclasses`);
			const data = await res.json();

			if (data.success && data.storageClasses) {
				allStorageClasses = data.storageClasses;
			} else {
				error = data.error || 'Failed to fetch storage classes';
				allStorageClasses = [];
			}
		} catch (err) {
			error = err instanceof Error ? err.message : 'Failed to fetch storage classes';
			allStorageClasses = [];
		} finally {
			loading = false;
		}
	}

	async function handleDelete(name: string) {
		if (!activeCluster?.id) return;

		try {
			deleting = true;
			const response = await fetch(
				`/api/clusters/${activeCluster.id}/storageclasses/${encodeURIComponent(name)}`,
				{ method: 'DELETE' }
			);
			if (!response.ok) {
				const data = await response.json();
				throw new Error(data.error || 'Failed to delete storage class');
			}
			toast.success(`Storage class "${name}" deleted`);
		} catch (err: any) {
			toast.error(`Failed to delete storage class: ${err.message}`);
		} finally {
			deleting = false;
		}
	}

	function openDetail(sc: StorageClassWithAge) {
		selectedSC = sc;
		showDetailDialog = true;
	}

	function openYamlEditor(sc: StorageClassWithAge) {
		drawerResource = { resourceType: 'storageclass', name: sc.name };
		showYamlDialog = true;
	}

	function closeYamlEditor() {
		showYamlDialog = false;
		drawerResource = null;
	}

	function handleYamlSuccess() {
		if (activeClusterId) fetchStorageClasses(activeClusterId);
	}
</script>

<svelte:head>
	<title>Storage Classes - AutoKube</title>
</svelte:head>

<section class="flex h-full min-h-0 flex-1 flex-col">
	<!-- Toolbar -->
	<div class="mb-4 flex shrink-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
		<div class="flex items-center gap-2">
			<h1 class="text-2xl font-bold">Storage Classes</h1>
			<span class="text-sm text-muted-foreground">
				{filteredSCs.length} of {scsWithAge.length}
			</span>
			<Button
				variant="outline"
				size="sm"
				class="h-7 gap-1.5 text-xs"
				disabled={loading || !activeCluster}
				onclick={() => { if (activeClusterId) fetchStorageClasses(activeClusterId); }}
			>
				<RefreshCw class={cn('size-3', loading && 'animate-spin')} />
				Refresh
			</Button>
		</div>
		<div class="flex items-center gap-2">
			<div class="relative flex-1 sm:flex-none">
				<Search
					class="pointer-events-none absolute top-1/2 left-2.5 size-3.5 -translate-y-1/2 text-muted-foreground"
				/>
				<Input
					placeholder="Search storage classes..."
					class="h-8 w-full pl-8 text-xs sm:w-56"
					value={searchQuery}
					oninput={(e) => scheduleSearch(e.currentTarget.value)}
				/>
			</div>
		</div>
	</div>

	<!-- Error -->
	{#if error}
		<div
			class="mb-4 flex items-center gap-2 rounded-lg border border-destructive/50 bg-destructive/10 p-4 text-destructive"
		>
			<AlertCircle class="size-4 shrink-0" />
			<span class="text-sm">{error}</span>
		</div>
	{/if}

	<!-- Empty states -->
	{#if !loading && !error && !activeCluster}
		<div class="flex flex-1 flex-col items-center justify-center gap-3 text-center">
			<div class="flex size-12 items-center justify-center rounded-full bg-muted">
				<Search class="size-6 text-muted-foreground" />
			</div>
			<div>
				<h3 class="mb-1 font-semibold">No cluster selected</h3>
				<p class="text-sm text-muted-foreground">Select a cluster to view storage classes</p>
			</div>
		</div>
	{:else}
		<div class="flex min-h-0 flex-1">
			<DataTableView
				data={filteredSCs}
				keyField="id"
				name={TableName.storageclasses}
				columns={storageClassesColumns}
				{sortState}
				onSortChange={(state) => (sortState = state)}
				onRowClick={openDetail}
				wrapperClass="border rounded-lg"
				virtualScroll={true}
			>
				{#snippet cell(column, sc: StorageClassWithAge, rowState)}
					{#if column.id === 'name'}
						<div class="flex items-center gap-1.5 text-sm font-medium">
							<Layers class="h-3.5 w-3.5 shrink-0 text-muted-foreground" />
							<span class="truncate">{sc.name}</span>
							{#if sc.isDefault}
								<Badge
									variant="secondary"
									class="ml-1 gap-0.5 px-1 py-0 text-[10px] text-amber-600 dark:text-amber-400"
								>
									<Star class="size-2.5" />
									default
								</Badge>
							{/if}
						</div>
					{:else if column.id === 'provisioner'}
						<span class="truncate font-mono text-xs text-muted-foreground">
							{sc.provisioner}
						</span>
					{:else if column.id === 'reclaimPolicy'}
						<Badge class="{getReclaimPolicyColor(sc.reclaimPolicy)} px-1.5 py-0 text-xs">
							{sc.reclaimPolicy}
						</Badge>
					{:else if column.id === 'volumeBindingMode'}
						<Badge class="{getBindingModeColor(sc.volumeBindingMode)} px-1.5 py-0 text-xs">
							{formatBindingMode(sc.volumeBindingMode)}
						</Badge>
					{:else if column.id === 'allowVolumeExpansion'}
						<div class="flex items-center">
							{#if sc.allowVolumeExpansion}
								<Check class="size-4 text-green-600 dark:text-green-400" />
							{:else}
								<X class="size-4 text-muted-foreground" />
							{/if}
						</div>
					{:else if column.id === 'age'}
						<span class="text-xs text-muted-foreground">{sc.age}</span>
					{:else if column.id === 'actions'}
						<div class="flex items-center justify-end gap-0.5">
							<Button
								variant="ghost"
								size="icon"
								class="h-6 w-6 text-muted-foreground hover:cursor-pointer"
								onclick={(e) => {
									e.stopPropagation();
									openYamlEditor(sc);
								}}
								title="Edit YAML"
							>
								<FileCode class="h-3.5 w-3.5" />
							</Button>
							<Button
								variant="ghost"
								size="icon"
								class="h-6 w-6 text-muted-foreground hover:cursor-pointer"
								onclick={(e) => {
									e.stopPropagation();
									openDetail(sc);
								}}
								title="View details"
							>
								<Info class="h-3.5 w-3.5" />
							</Button>
							<ConfirmDelete
								title={sc.name}
								loading={deleting}
								onConfirm={() => handleDelete(sc.name)}
							>
								<Button
									variant="ghost"
									size="icon"
									disabled={deleting}
									class="h-6 w-6 text-muted-foreground hover:text-destructive hover:cursor-pointer"
									title="Delete storage class"
								>
									<Trash2 class="h-3.5 w-3.5" />
								</Button>
							</ConfirmDelete>
						</div>
					{/if}
				{/snippet}

				{#snippet emptyState()}
					<div class="flex flex-col items-center justify-center py-16 text-muted-foreground">
						<div class="mb-3 flex size-12 items-center justify-center rounded-full bg-muted">
							<Layers class="size-6 text-muted-foreground" />
						</div>
						<h3 class="mb-1 font-semibold text-foreground">No storage classes found</h3>
						<p class="text-sm">This cluster has no storage classes</p>
					</div>
				{/snippet}

				{#snippet loadingState()}
					<div class="flex items-center justify-center py-16 text-muted-foreground">
						<Loader2 class="mr-2 h-5 w-5 animate-spin" />
						Loading storage classes...
					</div>
				{/snippet}
			</DataTableView>
		</div>
	{/if}
</section>

<!-- Detail Dialog -->
<Dialog.Root bind:open={showDetailDialog}>
	<Dialog.Content class="max-h-[90vh] max-w-4xl overflow-y-auto">
		<Dialog.Header>
			<Dialog.Title>Storage Class Details</Dialog.Title>
		</Dialog.Header>
		{#if selectedSC}
			<div class="space-y-6">
				<!-- Overview -->
				<div>
					<h3 class="mb-3 text-sm font-semibold">Overview</h3>
					<div class="grid grid-cols-2 gap-4">
						<div>
							<p class="text-sm font-medium text-muted-foreground">Name</p>
							<div class="mt-1 flex items-center gap-1.5">
								<p class="font-mono text-sm">{selectedSC.name}</p>
								{#if selectedSC.isDefault}
									<Badge
										variant="secondary"
										class="gap-0.5 px-1.5 py-0 text-xs text-amber-600 dark:text-amber-400"
									>
										<Star class="size-3" />
										default
									</Badge>
								{/if}
							</div>
						</div>
						<div>
							<p class="text-sm font-medium text-muted-foreground">Provisioner</p>
							<p class="mt-1 font-mono text-sm">{selectedSC.provisioner}</p>
						</div>
						<div>
							<p class="text-sm font-medium text-muted-foreground">Reclaim Policy</p>
							<div class="mt-1">
								<Badge
									class="{getReclaimPolicyColor(selectedSC.reclaimPolicy)} px-2 py-0.5 text-xs"
								>
									{selectedSC.reclaimPolicy}
								</Badge>
							</div>
						</div>
						<div>
							<p class="text-sm font-medium text-muted-foreground">Volume Binding Mode</p>
							<div class="mt-1">
								<Badge
									class="{getBindingModeColor(selectedSC.volumeBindingMode)} px-2 py-0.5 text-xs"
								>
									{selectedSC.volumeBindingMode}
								</Badge>
							</div>
						</div>
						<div>
							<p class="text-sm font-medium text-muted-foreground">Allow Volume Expansion</p>
							<div class="mt-1 flex items-center gap-1">
								{#if selectedSC.allowVolumeExpansion}
									<Check class="size-4 text-green-600 dark:text-green-400" />
									<span class="text-sm">Yes</span>
								{:else}
									<X class="size-4 text-muted-foreground" />
									<span class="text-sm text-muted-foreground">No</span>
								{/if}
							</div>
						</div>
						<div>
							<p class="text-sm font-medium text-muted-foreground">Age</p>
							<p class="mt-1 text-sm">{selectedSC.age}</p>
						</div>
						<div class="col-span-2">
							<p class="text-sm font-medium text-muted-foreground">Created</p>
							<p class="mt-1 font-mono text-sm">
								{formatCreatedAt(selectedSC.createdAt)}
							</p>
						</div>
					</div>
				</div>

				<!-- Parameters -->
				<div>
					<h3 class="mb-3 text-sm font-semibold">Parameters</h3>
					{#if Object.keys(selectedSC.parameters ?? {}).length > 0}
						<div class="max-h-48 space-y-1 overflow-y-auto">
							{#each Object.entries(selectedSC.parameters) as [k, v]}
								<div class="flex items-start gap-2 text-xs">
									<span class="min-w-0 font-mono break-all text-muted-foreground">{k}:</span>
									<span class="font-mono">{v}</span>
								</div>
							{/each}
						</div>
					{:else}
						<p class="text-sm text-muted-foreground">No parameters</p>
					{/if}
				</div>

				<!-- Labels -->
				<div>
					<h3 class="mb-3 text-sm font-semibold">Labels</h3>
					{#if Object.keys(selectedSC.labels ?? {}).length > 0}
						<div class="max-h-48 space-y-1 overflow-y-auto">
							{#each Object.entries(selectedSC.labels) as [k, v]}
								<div class="flex items-start gap-2 text-xs">
									<span class="min-w-0 font-mono break-all text-muted-foreground">{k}:</span>
									<span class="font-mono">{v}</span>
								</div>
							{/each}
						</div>
					{:else}
						<p class="text-sm text-muted-foreground">No labels</p>
					{/if}
				</div>

				<!-- Annotations -->
				<div>
					<h3 class="mb-3 text-sm font-semibold">Annotations</h3>
					{#if Object.keys(selectedSC.annotations ?? {}).length > 0}
						<div class="max-h-72 space-y-2 overflow-y-auto pr-1">
							{#each Object.entries(selectedSC.annotations) as [k, v]}
								{@const parsed = tryPrettyJson(v)}
								<div class="rounded-md border bg-muted/40 px-3 py-2">
									<p class="mb-1 font-mono text-[11px] break-all text-muted-foreground">{k}</p>
									{#if parsed.pretty}
										<pre
											class="overflow-x-auto font-mono text-[11px] leading-relaxed break-all whitespace-pre-wrap">{parsed.text}</pre>
									{:else}
										<p class="font-mono text-xs break-all">{v}</p>
									{/if}
								</div>
							{/each}
						</div>
					{:else}
						<p class="text-sm text-muted-foreground">No annotations</p>
					{/if}
				</div>
			</div>
		{/if}
	</Dialog.Content>
</Dialog.Root>

{#if activeCluster && showYamlDialog}
	<ResourceDrawer
		bind:open={showYamlDialog}
		clusterId={activeCluster.id}
		resource={drawerResource}
		onClose={closeYamlEditor}
		onSuccess={handleYamlSuccess}
	/>
{/if}
