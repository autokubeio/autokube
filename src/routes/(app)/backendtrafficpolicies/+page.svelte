<script lang="ts">
	...
	import type { BackendTrafficPolicyInfo } from '$lib/server/services/kubernetes';
	import { DataTableView, type DataTableSortState } from '$lib/components/data-table-view';
	import { TableName, backendTrafficPoliciesColumns } from '$lib/table-columns';
	import { toast } from 'svelte-sonner';
	import ResourceDrawer, { type ResourceRef } from '$lib/components/resource-drawer.svelte';

	type Row = BackendTrafficPolicyInfo & { age: string };

	const activeCluster = $derived(clusterStore.active);
	const activeClusterId = $derived(clusterStore.active?.id ?? null);
	let allItems = $state<BackendTrafficPolicyInfo[]>([]);
	...
	const filteredRows = $derived.by(() => {
		...
		if (sortState) {
			result = arraySort(
				result,
				sortState.field as keyof BackendTrafficPolicyInfo,
				sortState.direction,
				{
	...
	let watchHandle: ReturnType<typeof useBatchWatch<BackendTrafficPolicyInfo>> | null = null;
	...
	$effect(() => {
		...
		if (clusterId) {
			...
			watchHandle = useBatchWatch<BackendTrafficPolicyInfo>({
				clusterId,
				resourceType: 'backendtrafficpolicies',
				namespace: ns,
				getItems: () => allItems,
				setItems: (v) => {
					allItems = v;
	...
	async function fetchNamespaces(clusterId: number) {
		try {
			...
			console.error('[BackendTrafficPolicies] Failed to fetch namespaces:', err);
		}
	}
	...
	async function fetchItems(clusterId: number, nsParam: string) {
		...
		try {
			...
			const res = await fetch(`/api/clusters/${clusterId}/backendtrafficpolicies?namespace=${ns}`);
			const data = await res.json();
			if (data.success) {
				allItems = data.backendTrafficPolicies ?? [];
				crdMissing = !!data.crdMissing;
			} else {
				error = data.error || 'Failed to fetch BackendTrafficPolicies';
				allItems = [];
			}
		} catch (err) {
			error = err instanceof Error ? err.message : 'Failed to fetch BackendTrafficPolicies';
			allItems = [];
		} finally {
			loading = false;
		}
	...
	async function handleDelete(name: string, namespace: string) {
		...
		try {
			...
			const response = await fetch(
				`/api/clusters/${activeCluster.id}/backendtrafficpolicies/${encodeURIComponent(name)}?namespace=${encodeURIComponent(namespace)}`,
				{ method: 'DELETE' }
			);
			if (!response.ok) {
				const data = await response.json();
				throw new Error(data.error || 'Failed to delete BackendTrafficPolicy');
			}
			toast.success(`BackendTrafficPolicy "${name}" deleted`);
		} catch (err: unknown) {
	...
	function openYaml(r: Row) {
		drawerResource = { resourceType: 'backendtrafficpolicy', name: r.name, namespace: r.namespace };
		showYamlDialog = true;
	}
...
<svelte:head><title>BackendTrafficPolicies - AutoKube</title></svelte:head>
...
<section class="flex h-full min-h-0 flex-1 flex-col">
	<div class="mb-4 flex shrink-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
		<div class="flex items-center gap-2">
			<h1 class="text-2xl font-bold">BackendTrafficPolicies</h1>
			<span class="text-sm text-muted-foreground"
				>{filteredRows.length} of {rowsWithAge.length}</span
			>
			<Badge variant="outline" class="text-[10px]">Envoy Gateway</Badge>
			<Button
				variant="outline"
				size="sm"
	...
	{:else if !loading && !error && crdMissing}
		<div class="flex flex-1 flex-col items-center justify-center gap-3 text-center">
			...
			<div>
				<h3 class="mb-1 font-semibold">Envoy Gateway BackendTrafficPolicy not installed</h3>
				<p class="text-sm text-muted-foreground">
					This cluster does not have the <code class="font-mono">gateway.envoyproxy.io</code> CRDs installed.
				</p>
			</div>
		</div>
	{:else if !loading && !error && allItems.length === 0}
		<div class="flex flex-1 flex-col items-center justify-center gap-3 text-center">
			...
			<div>
				<h3 class="mb-1 font-semibold">No BackendTrafficPolicies found</h3>
				<p class="text-sm text-muted-foreground">
					{selectedNamespace === 'all'
						? 'This cluster has no BackendTrafficPolicies'
	...
	{:else}
		<div class="flex min-h-0 flex-1">
			<DataTableView
				data={filteredRows}
				keyField="id"
				name={TableName.backendtrafficpolicies}
				columns={backendTrafficPoliciesColumns}
				{sortState}
			...
			>
						...
						<div class="flex items-center justify-end gap-0.5">
							...
							>
								<Button
									...
									title="Delete BackendTrafficPolicy"
								>
				...
				{#snippet emptyState()}
					<div class="flex flex-col items-center justify-center py-16 text-muted-foreground">
						...
						<p>No BackendTrafficPolicies found</p>
					</div>
				{/snippet}
