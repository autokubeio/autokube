 ...
 */
export function getReplicaSetStatus(rs: ReplicaSet): string {
	if (rs.desired === 0) return 'Scaled Down';
	if (rs.ready === rs.desired && rs.desired > 0) return 'Ready';
	if (rs.ready === 0) return 'NotReady';
	return 'Partial';
}

/**
 * Status color mapping for replicaset status badges
 */
...
/**
 * Status icon mapping for replicaset status badges
 */
