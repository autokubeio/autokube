...
<Popover.Root bind:open>
	<Popover.Trigger>
		<Button variant="outline" size="sm" class="h-8 w-8 p-0" type="button">
			<CurrentIcon class="h-4 w-4" />
		</Button>
	</Popover.Trigger>
	<Popover.Content class="z-200 w-80 p-3" align="start">
		<div class="space-y-3">
			<Input bind:value={searchQuery} placeholder="Search icons..." class="h-8" />
			<div class="grid max-h-48 grid-cols-8 gap-1 overflow-y-auto">
				{#each filteredIcons as iconName}
					...
					<button
						...
						class="rounded p-2 transition-colors hover:bg-muted {value === iconName
