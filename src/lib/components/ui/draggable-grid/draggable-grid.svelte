...
<script lang="ts">
	...
	interface Props {
		...
		minH?: number;
		maxH?: number;
		/** Discrete snap steps for resize. When provided, resize snaps to the nearest step instead of arbitrary integers. */
		snapSizes?: Array<{ w: number; h: number }>;
	...
	/** Find nearest snap size by pixel distance → grid units */
	function findNearestSnap(pixelW: number, pixelH: number): { w: number; h: number } {
		if (!snapSizes || snapSizes.length === 0) return { w: 1, h: 1 };
		const rawW = pixelW / (colWidth + gap);
		const rawH = pixelH / (rowHeight + gap);
		let best = snapSizes[0];
		let bestDist = Infinity;
		for (const s of snapSizes) {
			const dist = Math.abs(s.w - rawW) * 1.5 + Math.abs(s.h - rawH);
	...
	function handleResizeMove(e: PointerEvent) {
		...
		const baseW = resizeStartW * colWidth + (resizeStartW - 1) * gap;
		const baseH = resizeStartH * rowHeight + (resizeStartH - 1) * gap;

		if (snapSizes && snapSizes.length > 0) {
			// Snap to nearest discrete tile-size step
			const snap = findNearestSnap(baseW + deltaX, baseH + deltaY);
			previewW = Math.max(minW, Math.min(maxW, Math.min(snap.w, cols - resizeItem.x)));
			previewH = Math.max(minH, Math.min(maxH, snap.h));
			// Drive the smooth visual delta to the snapped size
			resizePixelDeltaX = previewW * colWidth + (previewW - 1) * gap - baseW;
			resizePixelDeltaY = previewH * rowHeight + (previewH - 1) * gap - baseH;
		} else {
			// Free-form resize
		...
		>
			...
			<div class="tile-resize-handle">
				<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round">
					...
					<line x1="18" y1="11" x2="11" y2="18" />
					<line x1="18" y1="16" x2="16" y2="18" />
				</svg>
			</div>
		</div>
	{/each}
